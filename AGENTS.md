...
**HPC policy worth knowing:** HPC submit and compute nodes have outbound internet, so you can post to Slack. Issues begin when LLMs or other pipelines go to untrusted APIs or websites, do scraping, use skills or other agentic tools that may be malicious, or do any web browsing at all that would make them vulnerable to injection attacks. The rule is: **double-check with Roy if something you want to do is allowed on the HPC.** If it does local processing of, say, some HPC usage stats (like RoboRoy), and then the results are simply posted to Slack: that is fine. If any LLM-doing-stuff-with-or-on-the-internet-directly is involved, you can be sure that you should absolutely not do this. Be warned. Roy will come for you. (Run heavy/networked work through `sbatch`, not inline on a submit node.)
